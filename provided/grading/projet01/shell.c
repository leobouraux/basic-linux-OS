...
int do_exit(char args[ARG_NB][ARG_LENGTH]){
    // correcteur : il faut umount un éventuel disque monté et retourné la valeur de retour de umount
...
int do_mount(char args[ARG_NB][ARG_LENGTH]){
    M_REQUIRE_NON_NULL(args);
    // correcteur : il faut umount un éventuel disque monté
...
int do_cat(char args[ARG_NB][ARG_LENGTH]){
	...
	// correcteur : MAGIC NUMBERSSSS & pourquoi allouer la taille max si le fichier fait juste "coucou"
...
int do_sha(char args[ARG_NB][ARG_LENGTH]){
    ...
    // correcteur : il faut checker et retourner les éventuelles erreurs des couches
...
int do_inode(char args[ARG_NB][ARG_LENGTH]){
    M_REQUIRE_NON_NULL(args);
    // correcteur : il faut retourner les valeurs de retour dirlookup
    int inr = direntv6_dirlookup(&u, ROOT_INUMBER, args[1]);
    printf("inode: %d\n", inr);
    return 0;
}
...
int interprete(char args[ARG_NB][ARG_LENGTH], struct shell_map* current, size_t argnr){
    ...
    //if FS not mounted
    // correcteur : on aurait pu gérer ça de manière plus générique mais OK
