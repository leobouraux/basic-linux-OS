...
int inode_scan_print(const struct unix_filesystem *u){
    ...
    for(uint16_t inc = 0; inc < size; ++inc){
		...
		// correcteur : il faut afficher le numéro de l'inode et pas ce compteur qui est incrémenté uniquement si l inde est valide
...
int inode_read(const struct unix_filesystem *u, uint16_t inr, struct inode *inode){
    ...
    // correcteur : il faut aussi check la lowerbound aussi (<ROOT_INODE)
    if(inr > size) {
...
int inode_findsector(const struct unix_filesystem *u, const struct inode *i, int32_t file_sec_off){
	...
	// correcteur : gérer l'erreur de sector_read
