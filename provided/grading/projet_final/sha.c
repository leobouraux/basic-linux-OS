...
void print_sha_inode(struct unix_filesystem *u, struct inode inod, int inr) {
    ...
    if (inod.i_mode & IALLOC) {
        ...
        if (inod.i_mode & IFDIR) {
            ...
            int32_t inode_size_sector = inode_getsectorsize(&inod);

            char content[inode_size_sector];
            ...
            do{
                memset(currContent, 0, sizeof(currContent));
                //if the total content equals exactly 512 bytes, we ignore the new readblock
                if((rem = filev6_readblock(&filv6, currContent))>0)
                    strcat(content, currContent);
            } while (rem == SECTOR_SIZE);
            content[inode_size_sector-1] = '\0';
