...
int filev6_convert_to_big(struct unix_filesystem *u, struct filev6 *fv6){
    ...
    // correcteur : ce bm_set ne devrait être fait que tout à la fin avant le return 0 car si une erreur survient inutile d'utiliser le secteur
    ...
    if(inode_size % SECTOR_SIZE == 0){
        ...
        if(inode_size >= ADDR_SMALL_LENGTH * SECTOR_SIZE){
            if(inode_size % (ADDRESSES_PER_SECTOR * SECTOR_SIZE) == 0){
                ...
                // correcteur : ne traite pas tous les cas (écriture de gros fichiers depuis 0) mais je ne trouve pas pourquoi :S -1.5
...
int filev6_writebytes(struct unix_filesystem *u, struct filev6 *fv6, void *buf, int len){
    int offset = 0;
    int read_size = 1;
    // correcteur : en fait dans la condition, read_size ne serat à rien car vous avez un return dans le while, et quand ça vaut 0, c'est géré par l'offset
