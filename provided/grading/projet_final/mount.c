...
int mountv6(const char *filename, struct unix_filesystem *u){
    ...
    if(err >= 0){
        ...
        // correcteur : si une des allocatiosn échouue (== NUULL) il faut retouurner une erreur
...
int mountv6_mkfs(const char *filename, uint16_t num_blocks, uint16_t num_inodes) {
    ...
    if (f == NULL) {
        ...
        // correcteuur : Pour la root on aurait préféré ROOT_INUMBER
        sectorOfInodes[1].i_mode = IFDIR | IALLOC;
        // correcteur : pas besoin des 3 intilisations ci-après car {0} met tout à 0 et l adresse sera résolue dès le premier append
        ...
        for (uint32_t i = spb.s_inode_start; i < (uint32_t) (spb.s_block_start - 1); ++i) {
            // correcteur : attention, en faisant ça, le premier inode de chaque secteuur d inode est allouué alors qu'on ne vouulait que la root (il fallait créer un secteur `empty`)
